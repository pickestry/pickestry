...
export const Percent = React.forwardRef(({
  ...
  const onKeyDown = React.useCallback((e) => {
    ...
    // console.log(`DOWN Value is \`${e.currentTarget.value}\` keypressed is \`${e.key}\` cursor at ${start}:${end}[char: ${e.currentTarget.value.at(start)}][len: ${len}]`)
    ...
    if (controlUtils.isBackspace(e.key) && ((len - start) < 2)) {
      ...
      procOnChange(Math.trunc(+e.currentTarget.value * 100))
    ...
    if (e.key === '.') {
      // console.log('decimal, we move 1 space')
    ...
    if (start === end && le < 3) {
      const v = Array.from(e.currentTarget.value)
      const keyToUse = controlUtils.isDelete(e.key) && le === 1 ? '0' : e.key
      v.splice(start, 1, keyToUse)

      // console.log(keyToUse, v.join(''), start, end)
  ...
  const onChangeInner = React.useCallback((e) => {
    ...
    procOnChange(Math.trunc(+(e.currentTarget.value || 0) * 100))
  }, [])
...
const StyledPercent = styled.input`
  ${ cssInput }
  position: absolute;
  left: 0px;
  top: 0px;
